...
from app.metrics.recorder import ticket_telemetry_recorded
...
TICKET_METRIC_EVENTS: tuple[type[MetricLabelEvent], ...] = (
    TicketIssuedRecorded,
    TicketArtifactUploadRecorded,
    TicketEventConsumeRecorded,
    TicketEventPublishRecorded,
...
class TicketMetricsAdapter:
    def __init__(self, *, registry: CollectorRegistry, service_name: str, service_environment: str) -> None:
        ...
        self._metrics: MetricHandleMap = create_metrics_for_events(
            registry,
            service_label_names=TICKET_SERVICE_LABELS,
            event_types=TICKET_METRIC_EVENTS,
        )

    def connect(self) -> None:
        """티켓 telemetry signal과 Prometheus 기록 함수를 연결한다."""
        connect_metrics_for_events(
            ticket_telemetry_recorded,
            self._metrics,
...
def configure_ticket_metrics(
    registry: CollectorRegistry,
    *,
    service_name: str,
    service_environment: str,
) -> TicketMetricsAdapter:
    """ticket-service 전용 Prometheus metric adapter를 등록한다."""
    adapter = TicketMetricsAdapter(registry=registry, service_name=service_name, service_environment=service_environment)
    adapter.connect()
    return adapter
