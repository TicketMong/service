...
from app.metrics.events import (
    AuditEventRecorded,
    AuthAttemptRecorded,
    AuthTokenIssuedRecorded,
    AuthTokenRevocationRecorded,
)
from app.metrics.recorder import auth_telemetry_recorded
...
AUTH_METRIC_EVENTS: tuple[type[MetricLabelEvent], ...] = (
    AuthAttemptRecorded,
    AuthTokenIssuedRecorded,
    AuthTokenRevocationRecorded,
    AuditEventRecorded,
...
class AuthMetricsAdapter:
    def __init__(self, *, registry: CollectorRegistry, service_name: str, service_environment: str) -> None:
        ...
        self._metrics: MetricHandleMap = create_metrics_for_events(
            registry,
            service_label_names=AUTH_SERVICE_LABELS,
            event_types=AUTH_METRIC_EVENTS,
        )

    def connect(self) -> None:
        """인증 telemetry signal과 Prometheus 기록 함수를 연결한다."""
        connect_metrics_for_events(
            auth_telemetry_recorded,
            self._metrics,
...
def configure_auth_metrics(
    registry: CollectorRegistry,
    *,
    service_name: str,
    service_environment: str,
) -> AuthMetricsAdapter:
    """auth-service 전용 Prometheus metric adapter를 등록한다."""
    adapter = AuthMetricsAdapter(registry=registry, service_name=service_name, service_environment=service_environment)
    adapter.connect()
    return adapter
